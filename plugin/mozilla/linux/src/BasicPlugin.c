...
#define PLUGIN_NAME        "TvTide Plugin"
...
drawWindow(InstanceData* instanceData, GdkDrawable* gdkWindow)
{
  ...
  char* address = instanceData->addr;
  ...
  pango_layout_set_text(pangoTextLayout, address, -1);
  gdk_draw_layout(gdkWindow, gdkContext, x + 5, y + 5, pangoTextLayout);
  g_object_unref(pangoTextLayout);

  g_object_unref(gdkContext);
  
  /*system("cd /home/mantini/client/");
  strcpy(command,"/home/mantini/client/client-v-0.03 ");
...
{
  return "application/tvtide-plugin:bsc:TvTide plugin";
}
...
NPP_New(NPMIMEType pluginType, NPP instance, uint16_t mode, int16_t argc, char* argn[], char* argv[], NPSavedData* saved) {
  ...
  instanceData->addr = argv[1];
  instance->pdata = instanceData;

  strcpy(command,"exec ");
  strcat(command,getenv("HOME"));
  strcat(command,"/tvtide/sclient "); 
